...
def complete_from_env(timeout: float = 30.0) -> CompleteFn:
    """Build a bounded OpenAI-compatible ``complete`` from RAGBRAIN_LLM_* env (loads
    ...
    api_key = os.getenv("RAGBRAIN_LLM_API_KEY")
    base_url = (os.getenv("RAGBRAIN_LLM_BASE_URL") or "").rstrip("/")
    model = os.getenv("RAGBRAIN_LLM_MODEL")
