"""LlamaIndex bridge : adapters that put the ragbrain substrate behind a
...
LLM), so it is imported only when used - never from ``ragbrain.bridges.__init__``,
which keeps the core import-free. Validity is never re-implemented here; the
postprocessor calls the same ``ragbrain.core.policies`` definition as the substrate.
