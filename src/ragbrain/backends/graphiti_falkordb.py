...
it to the ragbrain ``AsyncSubstrate`` contract (write / read / falsify). The LLM
...
backend is actually used (never from ``ragbrain.backends.__init__``), keeping the
core import-free.
"""
...
_LOG = logging.getLogger("ragbrain")
...
class GraphitiFalkorDBConfig:
    ...
    # ("bge-m3", "nvidia-e5") are opt-in and require RAGBRAIN_EMBEDDER_API_KEY.
    embedder: str = "hash"
    ...
    @classmethod
    def from_env(cls, group_id: str = "ragbrain") -> GraphitiFalkorDBConfig:
        ...
        return cls(
            ...
            group_id=group_id,
            llm_api_key=os.getenv("RAGBRAIN_LLM_API_KEY"),
            llm_base_url=os.getenv("RAGBRAIN_LLM_BASE_URL"),
            llm_model=os.getenv("RAGBRAIN_LLM_MODEL"),
            embedder=os.getenv("RAGBRAIN_EMBEDDER", "hash"),
            embedder_model=os.getenv("RAGBRAIN_EMBEDDER_MODEL"),
            embedder_api_key=os.getenv("RAGBRAIN_EMBEDDER_API_KEY"),
            embedder_base_url=os.getenv("RAGBRAIN_EMBEDDER_BASE_URL"),
            overfetch_factor=int(os.getenv("RAGBRAIN_OVERFETCH_FACTOR", "10")),
            min_overfetch=int(os.getenv("RAGBRAIN_MIN_OVERFETCH", "50")),
        )
...
class GraphitiFalkorDBBackend:
    """Async :class:`~ragbrain.core.contracts.AsyncSubstrate` over Graphiti+FalkorDB."""
    ...
    async def read(self, query: RetrievalQuery) -> RetrievalResult:
        ...
        if self._last_read_saturated and not self._warned_saturated:
            ...
            _LOG.warning(
                ...
                "ranked below it may be missed. Raise RAGBRAIN_OVERFETCH_FACTOR / "
                "RAGBRAIN_MIN_OVERFETCH or narrow the query.",
                overfetch,
            )
